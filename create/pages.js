...
const pageTemplateFile = path.join(
  process.cwd(),
  "src",
  "templates",
  "index.js"
...
const createPages = async (pages, gatsbyUtilities) => {
  return Promise.all(
    pages.map((page) => {
      // If page has no dsas yet, initialise as empty array rather than null
      const components =
 ...
 *
 * Solution: Create a template file for every individual page which only imports the dsas
 * it requires.
 */
const createTemporaryPageTemplateFile = (
  ...
  // Use index.js as the base template to work from
  ...
  // Create the string which will import all the dsas this page needs
  ...
  // Create the string which sets the dsas variable
  ...
  // Create the string which will conditionally render all the dsas this page needs
...
/**
 * Crawls through all the fragment.js files in the dsas folders
